...
__global__ void eliminarBloques(int* tablero, int nRows, int nColumns, int coordY, int coordX) {
	...
	const int tamShared = 128;
	__shared__ int filaCompartida[tamShared];
	__shared__ int columnaCompartida[tamShared];
	...
	if (i < nRows && j < nColumns && i == coordY) {
		...
		while (start > 0 && filaCompartida[start - 1] == carameloElegido) start--;

		//Mientras haya caramelos iguales despues de nuestra posicion, aumentar la posicion de la columna de fin.
		while (end < nColumns - 1 && filaCompartida[end + 1] == carameloElegido) end++;
	...
	if (i < nRows && j < nColumns && j == coordX) {
		...
		while (start > 0 && columnaCompartida[start - 1] == carameloElegido) start--;
		while (end < nRows - 1 && columnaCompartida[end + 1] == carameloElegido) end++;
...
int main(int argc, char** argv) {
	...
	while (vidas > 0) {
		...
		if (tablero_host[coordY * columnas + coordX] == 10) {
			...
			activarRompecabezas << <blocks, threads >> > (tablero_dev, tablero_host[coordY * columnas + coordX] % 10, filas, columnas, coordX, coordY);
		}
		else {
